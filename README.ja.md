...
**大容量の写真・動画コレクション**（380 GB・38,485 ファイルで検証済み）を
...
> 💾 **写真アーカイブを何らかのツール（本ツールを含む）に任せる前に、
> バックアップを用意してください。** Sorta はバックアップが不要になるよう設計され
> ています: 振り分けは既定で dry‑run、移動は実行「前」にジャーナルへ記録され、
> `sorta undo` で戻せます。削除も `unlink` ではなくごみ箱行きです。とはいえこれは
> 設計であって保証ではありません。20 年分のたった 1 つのコピーは、作者自身の
> コードへの自信より重いものです。
...
> 快適に動きます**。
>
> **CPU プロファイルの基本ティアは測定済みです。モデルを使うステージは未測定です。**
> インデックス作成・位置の解決・近似重複・都市別整理は 40,295 ファイルで **24分51秒**
> でした（2026‑08‑22、論理 24 コア、プレビューキャッシュは空）。そのうち 19 分は全フレーム
> を初めてキャッシュへデコードする時間で、GPU プロファイルでも同じ作業に 19分02秒
> かかります。つまりカードがないことへの代償ではありません。CPU で**測定していない**のは
> モデルを動かすもの全て — 分類・顔・ランドマーク・深層ティアです。GPU はまさにそのために
> あり、推測は書きません。
...
| 速度 | **基本ティアを 2026‑08‑22 に実測**、40,295 ファイル、論理 24 コア、キャッシュは空: index 5分40秒 · geo 3秒 · phash 19分07秒 · dupes 1秒 — **24分51秒**。モデルを使うステージ（classify / faces / landmarks / 深層ティア）は CPU では**未測定** | 2026‑08‑05 実測、38,485 ファイルをゼロから: index 5.3分 · geo 3秒（オフライン）· landmarks 4.9 · classify 32.4 · faces 14.2 · events 2秒 · junk 19.3 · phash 45秒 — 合計 **77.5分**。プレビューキャッシュが温まっていれば 10 分ほど短縮。`phash 45秒` は順番と併せて読んでください。5 番目に走るため、先行ステージが全フレームをキャッシュへデコード済みです。先頭に置けばどちらのプロファイルでも 19 分かかります |
| 向いている用途 | 任意のマシンでの都市別振り分け + 重複検出、顔検出/イベントを有効にした小規模コレクション | 顔検出/イベントを常用する大規模コレクション — 上の実測は **380 GB / 38,485 ファイル** |
...
Sorta は NVIDIA GPU を前提にしています。低電力の家庭サーバー上のコンテナでは、
基本ティア（インデックス、ジオコーディング、重複検出、都市別の振り分け）しか動かず、
このツールを単なるフォルダ整理スクリプトと区別する部分 — CLIP による分類、顔の
クラスタリング、言葉による検索 — は使えません。残りを動かせないマシンに製品の
最も退屈な半分だけを渡すのは良い取引ではないので、イメージはありません。見落としでは
なく、判断です。

障害はパッケージングではありません。Web アプリは `127.0.0.1` にのみバインドし、
認証を持ちません。localhost では必要なかったからです — しかしその画面には顔、書類、
そして削除ボタンが並んでいます。これをネットワークに出すかどうかは製品としての問いであり、
その答えは Dockerfile の後ではなく前に必要です。

イメージが必要な場合は issue を立て、どんなハードウェアで動かすつもりかを書いて
ください。それこそが、この判断に今欠けている測定値です。
...
`exiftool` も同梱されるため、HEIC/RAW/動画の日付と GPS は最初の実行から読めます。
ショートカットはトレイアイコン付きのウェブアプリを開き、コンソール窓は出ません。
重いティア — コマの内容の認識（約 1.6 GB）、顔（約 400 MB）、言葉による検索（約 1.4 GB）、
NVIDIA/CUDA 13（約 2.5 GB）、ディープ VLM ティア（約 7 GB）— は初回セットアップ
（`sorta-setup`、いつでも再実行可）が一度だけ尋ねます。**すべて断っても、削られた製品では
なく動く製品が残ります**。**「はい」と答えたティアはその場ですべてダウンロードされ**、
その場で描き直される 1 行の進捗表示が付きます。初回実行の途中ではありません。断ることは
引き続き可能で、変わるのは取得のタイミングだけです。最初のティアの上に振り分けが成り立つ
ため、既定の答えが**「はい」**になっているのはそれだけです。NVIDIA の行は**カードがあり、
CUDA 13 に足りるドライバがある場合にのみ**提示されます。これは動いている CPU プロファイル
に足すのではなく置き換える唯一のティアで、`sorta-setup --restore-cpu` でそのプロファイル
に戻せます。プログラムの削除はスタートメニューの **Uninstall Sorta** から行い、その画面は
設定とダウンロード済みモデルを別々に尋ね、どちらも既定の答えは「いいえ」です。
...
sorta-tray                              # ...同じものをトレイアイコン付きで（エクストラ: tray）
...
`sorta-tray`（F207）は、通知領域にアイコンを置く同じ Web アプリです — ターミナルではなく
ショートカットから起動した Sorta のためのもの。アイコンのダブルクリックか **開く** で
ウィンドウが開き、**終了** でプログラムが閉じます（実行中なら先に確認します）。
`tray` エクストラが必要です（`…sorta[gpu,tray]`）。エクストラがない場合も、トレイのない
デスクトップでも、アイコンが出ないだけでサーバーは動きます。

**起動時に何が起きるか**（F227）: **「Sorta を起動しています」**の小さなウィンドウが、
プログラム本体が読み込まれる*前*に出るので、最初の数秒が空の画面になることはありません。
インターフェースは**サーバーが応答できるようになった時点で**引き渡され、環境とグラフィック
カードの点検はポートのバインドの前ではなく後ろで走ります。そして**ショートカットの 2 回目の
クリックは、すでに動いているコピーのタブを開きます** — 2 つ目のプロセスではありません。
ポートの確認が最初に来て、重い読み込みはその後だからです。
...
- **デフォルトは dry‑run。** すべての操作は実行前にジャーナルへ記録され、
  `sorta undo` で取り消せます。
- **削除はごみ箱へ。** レビュー画面はファイルを `send2trash` に渡すだけで、
  OS はごみ箱を空にするまで保持します。区別が重要です: `undo` は移動ジャーナルを
  逆再生するものなので、削除したファイルはごみ箱から戻り、`undo` からは戻りません。
- **それでもバックアップを。** 以上はすべて設計です。設計が誤っていた場合を
  埋めるのがバックアップです。
...
- `CONTRIBUTING.md` — 貢献方法 · `SECURITY.md` — プライバシーと報告 ·
  `NOTICE` — サードパーティデータの帰属表示と、Sorta がダウンロードする
  モデル重みのライセンス（うち 2 つは非商用の研究目的のみ）
...
独自の帰属表示要件があり、実行時にダウンロードされるモデル重みにもそれぞれの
ライセンスがあります。顔モデルと深層ティアの VLM は**非商用の研究目的のみ**です。
どちらのステージもオプトインです。[NOTICE](NOTICE) §5 を参照してください。
