...
uv run --extra cpu --extra dev python scripts/check.py --slow  # the test suite, ~7 min
...
The slow half runs the suite **twice**: everything in parallel (`pytest -n`, whole files
at a time, **at most 8 workers**), then the handful of tests marked `serial` in a single
...
not a place to put a test that went red once**: a failure that only happens in parallel
is shared process state until proven otherwise, and it gets fixed rather than marked.
Every use carries its reason next to it, and `tests/test_gate_parallel.py` fails the gate
if one does not.

The eight is a **memory** budget, not a core count: under `--extra gpu` each worker
carries a 505 MB torch, and one worker per core exhausted a 63 GB machine that had a
product run and another gate on it. Seven minutes is what the two passes plus the
coverage report took on a 24-core machine on **both** profiles, against **30 minutes**
in one process — 4.4x, where the uncapped version was 7.5x and did not finish on the gpu
profile at all. `scripts/measure_gate_workers.py` re-derives the table behind the cap.
The run prints its own duration at the end, so the next time this number goes stale it
is visible on the spot.
