...
| ui/cli | `cli.py`, `ui/` (F182: a package cut by TAB — `common`, `layout`, `slices`, `review`, `overview`, `moves`, `process`, `page`, `strings`, and the server in `__init__.py`; the markup/styles/script live in `web/`), `tray.py` (F207: the second entry point — an icon in the tray for the installed application; it starts `ui.build_server` and quits through `POST /api/quit`, so it adds no behaviour of its own) | everything (read) | `manual_overrides`, `manual_places`, `manual_pet`, `dedup_choice` — the user's OWN decisions; otherwise orchestrate module calls |
