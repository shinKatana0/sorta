...
**1 つだけはセットアップ中にダウンロードします** — 表の最初の行です。振り分けは分類の上
に成り立ち、実行中に分類の可否を尋ねられることはありません。だからその 1.6 GB はここで
取得します: 人が画面の前にいて、進捗が行として表示され、失敗もその場で説明できるから
です。断ることは引き続き可能で、失うのはタイミングだけです — 同じファイルをステージが
初回実行時に取得し、実行画面が開始前にそれを伝えます（§6）。ダウンロードが失敗しても
（ネットワークなし、証明書）、インストールはそのままで、プログラムは動作します。
