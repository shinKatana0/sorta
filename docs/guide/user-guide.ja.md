...
| `features.detector` | `false` | 動物の印に**第 3 の段: 物体検出器**（COCO、torchvision）を、検索クエリが選んだ候補に対してかけるかどうか。`detect.enabled` が必要です — `vlm.enabled` ではなく専用のマスタースイッチで、検出器は VLM ではないからです。これは**測定した 3 つの切り口のうち 1 つ**のためだけに存在します: 手作業でラベル付けした 200 コマ、信頼度 0.5 で、検出器は動物について適合率 62% / 再現率 87%、いまパイプラインが書いている印の 71% / 33% に対して。一方で人については適合率 42%（顔ボックスの約 100% に対して、§6）、食べ物については 20% / 15% です — COCO には `food` クラスが存在せず、バナナ・ピザ・サンドイッチしかないからです。したがって人と食べ物はここでは意図的に検出**しません**。これは全件処理ではなくカスケードです: 22 096 枚に検出器を通すと実測 83.8 ミリ秒/コマで 30.8 分、クエリの候補約 2 000 コマなら約 3 分です。答えは CLIP の印を**両方向に**上書きします — スコアが低くて印が付かなかったコマに動物が見つかれば印が付き、CLIP が動物と呼んだのに何も検出されなければ印が外れます — そして拒否（保存済みベクトルがない、重みがない、1 コマでのエラー）はすべて、いまある印にそのまま戻り、決して「動物なし」にはなりません。見つかったものはクラス・信頼度・座標とともに保存され（テーブル `detections`）、「猫を犬と分けて」といった切り口は別の全件処理ではなくクエリになります。`features.store_embeddings` が必要です: 保存されたベクトルがなければ候補もなく、その場合ステージは全件処理に落ちるのではなく、その理由を伝えます。 |
| `features.detector_candidates` | `2000` | クエリの順位のどこまでを候補にするか — 検出器はそれ以外を見ないので、費用を決める**唯一の**数字です。2 000 コマは実測 83.8 ミリ秒/コマで約 3 分。再現率はこの深さでのクエリ自身の再現率（ラベル付き標本で 87%）が上限なので、増やせばクエリが下位に置いた動物が拾えるようになり、時間は線形に増えます。`python scripts/measure_detector.py` が自分のコレクションでの深さごとの上限を表示します。 |
| `features.detector_threshold` | `0.5` | 検出ボックスを動物と数える信頼度。**事前に決めず、表から選んでください:** `python scripts/measure_detector.py` が自分のラベル付き標本で 0.3 / 0.5 / 0.7 の適合率と再現率を、同じコマでの現行 CLIP の印の成績と並べて表示します。`0.5` が実測された行です（62% / 87%）。この規則の教訓は `features.pet_candidate_threshold` で、事前に提案された値が表の最悪の行になりました。各ボックスはスコアとともに保存されるので、しきい値の選び直しはクエリ 1 回で済み、画像への新たな処理は不要です。 |
