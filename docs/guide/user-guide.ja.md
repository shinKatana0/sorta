...
- UI では **フォルダに集める** ボタン: 人物／イベントのカード、**動物**のスライス、
  分類のバケット（商品・スクリーンショット・ミーム）、そして**見直し**の画質
  スライス（ぼやけ・目を閉じた・低解像度）。どこでも同じ行です: モード、任意の
...
| キー | 何をするか |
|---|---|
| `vlm.enabled` | **モデルのマスタースイッチであり、それ以上のものではありません。** 既定は `false`。VLM の読み込みを許可するだけで、これ自体は何も実行せずコストもゼロです。モデルへの質問はそれぞれ専用のキー(`vlm.products`、`vlm.quality`、`features.pets_verify`、`features.landmarks_verify`、`dedup.keeper_vlm`、`features.junk_rescue`)で有効になり、このキーは「尋ねる相手がいるかどうか」を決めます。1 回の実行だけなら `--deep`/`--no-deep` と Web UI の「詳細解析 (VLM)」チェックボックスが同じ働きをします。`vlm` エクストラが無い場合は高速 CLIP ティアへ自動的にフォールバックします。 |
...
| `vlm.quality` | フレームの品質に関する質問（目が開いているか）専用のトグルです。既定は `false`。もう一つの質問「偶然の一枚ではないか」は F122 まで尋ねていましたが、廃止しました — ラベル付き標本での正答率は 5%、つまり雑音でした。目に関する答えは、顔検出器が顔を見つけたコマでのみ採用します。鮮明さとペットはこれなしで求まります — ラプラシアンと CLIP はどちらも無料で、モデルにはそのどちらでも決められないことだけを尋ねます。 |
...
はローカルの VLM に尋ねるもので（`vlm.quality`）、時間がかかるのはこちらです。誰に尋ねるかは `vlm.quality_scope` が決めます:
