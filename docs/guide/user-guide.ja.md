...
- **セレクタを取らないスライス**: `animal`、`product`、`screenshot`、`meme`、
  `blurred`、`eyes_closed`、`people`、`group`、`portrait`。その中に選ぶ
...
- UI では **フォルダに集める** ボタン: 人物／イベントのカード、**動物**のスライス、
  分類のバケット（商品・スクリーンショット・ミーム）、そして**見直し**の 2 つの画質
  スライス（ぼやけ・目を閉じた）。どこでも同じ行です: モード、任意の
...
区分は 3 つで、上部のボタンで切り替えます。各ボタンには、まだ決めていないコマの件数
が付きます:
...
されても二度と尋ねられません。記録は 3 つの区分で共通なので、「ぼやけ」と「目を
