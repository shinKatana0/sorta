...
| `features.restore_max_edge` | `1024` | そのモデルに渡す**前に**コマを縮小する長辺の上限で、結果として何が返ってくるかを決めるただ一つの数字です。上限**より小さい**コマはそのままモデルに渡されます — 入力 800 px、出力 3200 px、失われるものはありません。これがこのボタンの本来の用途です（スキャン、ダウンロードした画像、古い小さな写真）。上限**より大きい**コマはまず縮小され、×4 がほぼ元の大きさに戻します: `4032 → 1024 → 4096`。出力の大きさは同じですが、元の写真の本当の細部はモデルに届かず、代わりにそれらしい細部が描き足されています。だから**「仕分け」**タブは複製の隣でそのことをそのまま告げ、改善とは呼びません。上限がある理由は、モデルが**拡大後**の解像度で計算するからです: 4000 px のコマは内部で 16 000 px、約 780 メガピクセルになり、32 GB でも収まりません。上限を上げる代償はこの数字の**二乗**でメモリに効きます。動かす前に `python scripts/measure_restore.py` を実行してください。コマの母集団ごと（< 1024 px、1024〜2500 px、> 2500 px）に時間・容量・ピークメモリを、常に基準となる元の写真と並べて表示し、「元 / 処理済み」の盲検ペアを並べます。複製が本当に良いかを判断できる唯一の正直な審判は、それを見る人です。「くっきりした」と「良くなった」は別のことです。 |
...
示されることはありません。
**大きなコマでは、その複製が何であるかを必ず伝えます。** モデルは ×4 で、12 メガピクセル
のコマ全体を渡すことはできません。そうしたコマはまず `features.restore_max_edge`（§21）
まで縮小され、ほぼ元と同じ大きさで戻ってきます — 元の写真から鮮明にしたのではなく、
縮小した複製から作り直したものです。これは黙って行われる取引ではありません。「完了」の
すぐ隣で、両方の数字とともにその旨を告げます。上限より小さいコマでは失われるものはなく、
何も告げられません。
