...
> どう見えるかは §7 と §9 の実例を、`????` のように文字化けする場合は §22 を参照して
> ください。
...
1. **概要** タブ → コレクションの状態を 1 画面で表示: インデックス内のファイル数、
   ...
   そのグループを操作できるタブへ移動します。常に表示され、インデックスが空のときは
   代わりに **処理タブへ** ボタンが出ます。
2. **処理** タブ → 写真フォルダのパスを入力。2 つのチェックボックスは既定で**両方
   ...
   **処理する** をクリックすると、パイプラインがバックグラウンドで段階別プログレス
   ...
   処理は継続します。
3. **都市** タブ → 提案された構成を確認（`国/都市/年/地区`）。**設定**の列（後述）と
   振り分けを開始するボタンもここにあります。常に表示されます。
4. **重複** タブ → 類似写真グループを確認。おすすめの残す 1 枚（★）が事前選択済み。
   異なる場合だけ変更し、一度だけ **すべての選択を保存** をクリック。常に表示され
   ます。
5. **人物** タブ → 顔クラスタが存在する場合のみ表示されます（一度でも「顔を検出」
   にチェックを入れたか、`sorta faces` を実行した場合）。クラスタに名前を付け、
   同一人物の重複クラスタを統合。
6. **イベント** タブ → イベントが存在する場合のみ表示されます（一度でも「イベント
   を検出」にチェックを入れたか、`sorta events` を実行した場合）。イベント名を編集。
   任意の人物／イベントを **フォルダに集める** でフォルダ化。
7. **個人写真ではないもの** タブ → 分類器が思い出から外したバケット:
   **商品**、**書類**、**スクリーンショット**、**ミーム**（および「すべて」）。誤って
   ここに入ったフレームにチェックを入れて **写真に戻す** を押すと、再び都市別の
   振り分けに戻ります。モデルの判定自体は書き換えられず、戻す操作そのものも
   **戻すのを取り消す** で元に戻せます。常に表示されます。
   **書類にはサムネイルを表示しません** — ファイル名と日付だけです。これは未完成では
   なくルールです: このバケットにはパスポート、証明書、診断書が入るため、Sorta は
   ローカルであっても開かず描画しません。判断にはファイル名と日付で十分です。
8. **移動** タブ → 振り分け／アルバム適用後、何がどこへ行ったかを確認。振り分けの
...
**都市** タブの右側に **設定** の列があります。ここでの変更は本当に `config.yaml` に
...
処理の実行中は設定を変更できません。その間、列は実行中である旨を表示し、終了まで
待つよう促します。
...
sorta junk                        photo/screenshot/meme/document/product を分類
...
| `vlm.quality` | フレームの品質に関する質問（目が開いているか、残す価値のある被写体があるか、偶然の一枚ではないか）専用のトグルです。既定は `false`。鮮明さとペットはこれなしで求まります — ラプラシアンと CLIP はどちらも無料で、モデルにはそのどちらでも決められないことだけを尋ねます。 |
