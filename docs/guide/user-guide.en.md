...
| Base tier speed | **Measured 2026‑08‑22** on 40 295 files (26 447 canonical), 24 logical cores, cold preview cache: index 5 min 40 s · geo 3 s · phash 19 min 07 s · dupes 1 s — **24 min 51 s** for everything that gets a collection sorted by city. Nineteen of those minutes are the first decode of 24 504 frames into the preview cache; the GPU profile spends **19 min 02 s** on the same work, so that part is not a CPU penalty at all. Errors: 0 | The same stages inside the 2026‑08‑05 run (§8), where `phash` costs 45 s because it runs fifth and the cache is already full |
...
|---|---|---|---|
| recognising what is in a frame | ~1.6 GB | ViT‑L‑14: photographs told apart from screenshots, documents and product shots — **what the layout runs on**; the same weights serve places by sight and search by words | the classification stage fetches the same 1.6 GB itself, on the first run and in the middle of it |
| faces | ~400 MB | buffalo_l: face detection, clusters, per‑person albums | sorting by people is unavailable; everything else works |
| search by words | ~1.4 GB | XLM‑RoBERTa on top of the tier above: `sorta search`, pinned slices | search says it has no index; the face and class slices, the verdicts and the city layout are untouched |
...
XLM‑RoBERTa, so half of it lives in the tier above. Choosing search by words brings that
tier along; a machine that already has it downloads nothing twice.
...
sorta album low_resolution --dest /path/to/albums --apply
...
- **Slices without a selector**: `animal`, `product`, `screenshot`, `meme`, `blurred`,
  `eyes_closed`, `low_resolution`, `people`, `group`, `portrait`. There is nothing to choose inside them
...
sorta album blurred|eyes_closed|low_resolution --dest DIR [--copy|--move] [--where …] [--name N] [--apply]
...
| `features.saved_slices` | `children` / `products` / `animals` | The **pinned slices** of the **Slices** tab: a name and the English phrases that slice is ranked by. A slice here is a **saved query**, not a filter with a threshold of its own — the vectors are the ones `features.search_index` already stores, and asking them costs one matrix multiplication (0.9 ms measured). Measured on 2026‑08‑02 on 200 hand‑labelled frames out of 22 096, the first time **recall** was measured and not just the precision of the top: children **61%** recall in the first portion and **89%** in a doubled one (there was no filter for them at all), products **65% / 95%** (the classifier's own `product` verdict found **0** of them), animals **60% / 87%** against **33%** for the `pets` label. That label stays and is shown beside the query slice: at 71% precision it answers "is this confidently an animal", which is a different question from "show me every animal", so the interface labels the two apart (**Animals** against **Animals · by query**). **People are not here** — the `faces` table answers that exactly and for free (§21) — and neither is **blurred**, where the sharpness filter is 100% precise on that sample against 36% for the query. **The phrases are English and should stay English**: they are not interface text, they go to a CLIP text tower, so `language:` does not touch them and the numbers above were produced by this wording. **How many phrases does not matter** — one, three and six differ by less than the noise of the sample — the list is there so a slice can be retuned, added or deleted **without code**. There is no membership threshold: the list is ranked, and **Show more** continues it `features.search_page` frames at a time. |
| `features.max_pinned_slices` | `12` | How many slices the **web interface** may pin into `features.saved_slices` above. **Not a resource bound** — a pin costs a line in the config file and one matrix multiplication when it is opened — but the same bound F133 applied to the screen when it removed thirteen controls from it: forty pins is a remote control again, and the slice you actually wanted becomes the hardest thing on the tab to find. Reaching it is **said out loud** («N slices are pinned — that is the limit»), never a pin that quietly does not appear. Editing `config.yaml` by hand is **not** governed by it: a longer list written there keeps every slice, and only the button stops adding more. |
...
| `features.restore_max_edge` | `1024` | The longer side a frame is scaled to **before** that model, and the single number that decides what you get back. A frame **below** it goes to the model untouched — 800 px in, 3200 px out, nothing given up, and that is the case the button was built for (a scan, a downloaded picture, an old small shot). A frame **above** it is **refused**: it would have to be reduced first and the ×4 would bring it back to about its own size (`4032 → 1024 → 4096`), the real detail of the original never reaching the model — and on blind pairs that came back indistinguishable from the original, so the copy would cost a run of the model and a near-duplicate file to buy nothing. The refusal names this limit, the size of the frame and this key, because the threshold is yours to move. The limit exists because the model works at the **upscaled** resolution: a full 4000 px frame would be 16 000 px wide inside it, around 780 megapixels, which does not fit on 32 GB. Raising it costs memory as the **square** of the number. Before you move it, run `python scripts/measure_restore.py`: it prints time, weight and peak memory per frame population (< 1024 px, 1024–2500 px, > 2500 px), always against the original as the baseline, and lays out blind pairs of “original / processed” — the only honest judge of whether the copy is better is a person looking at it, and “sharper” is not “better”. |
...
- **Closed eyes** — frames where the people most likely have their eyes closed, most
  ...
  §21). On a hand-labelled sample the list is **right 62% of the time** — one frame in
  three here has its eyes open — so nothing is ever marked or deleted automatically, and
  the button continues past the threshold into the doubtful part. Measured only where the
  detector found a face, so without a `faces` run this slice is empty and says so.
- **Low resolution** — photographs of fewer than `features.low_resolution_mp` megapixels
  (default `1.0`), the smallest first, with the size printed on every card
  (`1280×960 (1.2 MP)`).

**Low resolution is the one slice here that measures nothing.** The width and the height
are written down when the file is indexed, so this list does not find frames that *look*
small — it enumerates the frames that *are* small, and there is no threshold anybody
calibrated and no accuracy to quote. On a collection of 22,095 photographs it holds 706
frames, and 682 of them are formally sharp: no other slice shows them at all, which is
why it is a slice of its own rather than a corner of "Blurred".

**Small is not faulty.** Such a frame can be the only surviving photograph of somebody,
sent through a messenger ten years ago, so the slice is named after the fact and nothing
in it is marked for deletion by default. Two limits worth knowing: megapixels say nothing
about a **large frame ruined by compression** (a 4000×3000 picture full of JPEG artefacts
is not here — that is a different signal), and **videos are not counted**, because their
resolution has its own meaning. This is also the slice **"Try to improve"** was really
built for: Swin2SR ×4 is a super-resolution model, and 640×480 becomes 2560×1920.
...
journal is shared by every slice, so a file that turns up both in "blurred" and in
...
card beside the original**, marked as processed, with the same actions on it — so you
compare two pictures rather than read a message saying a file was saved. Keep either,
both or neither; choosing the copy does not mark the original for deletion, because those
are two decisions and the second one is yours.
**The model does not bring back what was lost — it draws something plausible**, which for
an archive is more dangerous than the blur: a smeared frame is honestly smeared, a redrawn
face looks real and is not. So the original is never touched, the copy says what it is in
its name (`<name>_restored.jpg`, beside the original, never over an existing file) and on
its card, and there is no way to run this over more than one frame — no bulk button, no
stage, no CLI command. The first press downloads ~400 MB of weights (they need the `[vlm]`
extra); after that it is about a second per frame, and while it works the button says so.
Offline and without cached weights it tells you that instead of returning an empty result.
Press it twice on the same frame and you get the copy you already have, not a second one.
...
**On a full-sized frame the action refuses.** The model is ×4 and cannot be shown a whole
12‑megapixel shot: such a frame would be scaled to `features.restore_max_edge` (§21) first
and come back at about its own size — rebuilt from a reduced copy of itself rather than
sharpened from the original — and on blind pairs above that limit the measurement found no
gain at all. So the button is not offered there, a request made past it is refused as well,
and the answer names the limit, the size of this frame and the key that moves the limit:
the threshold is yours to change. Nothing is written for such a frame. On a frame under the
limit nothing is given up and nothing is said. Copies made before this became a refusal
stay where they are — removing them is your decision, not something a fix does for you.
...
**Pinned slices: children, products, animals by query.** Some queries are worth keeping,
so the **Slices** tab pins three of them beside people and events — you do not have to know
that a query is what is under them. They are **estimates and are labelled as such**: the
list is ranked, no model has checked it, and a pin says `· by query` so it is never mistaken
for an exact slice (**Animals · by query** stands next to **Animals**, which is the `pets`
label — 71% precision, verified by a model — and the two answer different questions). The
panel prints the phrases the slice asked, and they live in `features.saved_slices` (§21):
edit a phrase, add a slice, delete one — no code, no release. **Show more** is the main
button here: on the sample this was measured on, doubling the list took children from 61% to
89% completeness.

**Pin a query of your own.** The three above are a starting point, not the answer: on a
random sample of 200 frames a third of them fell into no class at all, and the ten
candidate slices for those 65 frames covered 26%, 23%, 22%, 20%, 18%, 17%, 15%, 12%, 12%
and 6% of them — not one reached a third of a third, and food, which everybody expected to
be large, came out at 8 frames. There is no such thing as *the* set of facets: for one
person they are mountains and children, for another receipts and cars. So the product does
not guess. Search for something, and when there are results the **Pin as a slice** button
appears beside the line: it asks for a name (the query itself is offered) and the query
becomes a pin like any other — the same grid, the same **Collect into folder** with link /
copy / move, the same **Show more**. Four things worth knowing:

* the pins are stored in `config.yaml` (`features.saved_slices`), **not** in the index, so
  they survive `sorta reset` and every re-processing;
* **Unpin the slice** sits inside the slice and asks first. It removes the pin and **not
  one photograph**;
* the arrows on the panel move a pin up and down the row; the order is the file's order;
* there are at most `features.max_pinned_slices` of them (12), and reaching that is said
  out loud rather than being a button that does nothing.

**A pinned query is an estimate, exactly like the three above**: about 60% of what you are
after in the first portion, about 90% in a doubled one, and precision falls as depth grows
— somebody who pins «mountains» will get hills and clouds too. And **the wording goes to
the model as it stands**: the search index is English until the multilingual one is built
for your collection, so a Russian or Japanese pin will work noticeably worse — the
interface says so at the moment you pin it, not a week later.

**Slices that are not queries do not unpin, and are never hidden by hand.** People,
events, animals and duplicates are the core of the product, and there is already one
control for them: the checkboxes on the run screen. Do not need animals — do not compute
animals, and the slice is not there. A second way to take the same thing off the screen
would inevitably disagree with the first, and you would be left guessing whether a slice
is missing because you hid it or because you never computed it. What such a slice **does**
owe you is an explanation of its own emptiness, and it gives one: **"this was not
computed"** with a link to the run screen when the stage has not run, and **"computed, none
found"** when it has. A bare zero reads as the second when it is nearly always the first.
