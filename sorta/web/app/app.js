  ...
  function browseIntoField(btn, apply) {
    ...
    postJson("/api/browse", {})
      .then(function (resp) {
        ...
        // A refusal answers `problem`, and saying nothing there is what made the button
        // look broken on Ubuntu. A cancel still answers nothing at all.
        // F247: which refusal it was, keyed on the CODE and never on the text — a dialog
        // that ran and lost its answer must not be reported as a machine without one.
        if (resp && resp.problem) {
          window.alert(resp.problem === "no_answer"
              ? I18N.browse_no_answer : I18N.browse_unavailable);
        }
      })
      .catch(function () {})
