...
`exiftool\\exiftool.exe`. `sorta.wizard` has read that file since F211 and was for a while
its only reader, so the reading lived there.

The lookup moved here when the metadata reader needed the same answer: `sorta.exif`
asking `sorta.wizard` where exiftool is would import the whole tier catalog — six tiers,
their sizes, their translated names, a screen — to resolve one path, and would make the
module that reads EXIF depend on the module that installs CUDA.
...
the cleanup that follows; it is not part of this feature.
"""
...
# Paths inside the manifest are written RELATIVE to it; this key remembers the directory
# it was read from. The build cannot know where a person will install the program, and a
# relative path needs no install-time rewriting to survive being copied somewhere else.
MANIFEST_ROOT = "root"
...
def manifest_path(explicit: str | Path | None = None) -> Path | None:
    ...
    environment names, and the directories above the running interpreter. A checkout has
    none of the three and gets None — there is no install there to have shipped anything.
    """
    if explicit:
...
def load_manifest(explicit: str | Path | None = None) -> dict:
    ...
    A broken manifest may not break the product: every caller has a fallback behind it
    (the reader falls back to Pillow, the wizard probes), and an install that answers
    "nothing was shipped" degrades like a checkout, a state the program already handles.
    """
...
# of the paths below, and each such line used to be written for whichever path its author
# happened to be on. Three were found by the owner in a virtual machine over two days —
# `uv sync --extra gpu --extra dev` printed by `doctor` to a copy that has no project
# directory, `uv sync --extra vlm` in the help of `--deep`, and a tier hint that offered
# the Start menu to a developer because it was keyed BY OPERATING SYSTEM.
#
# That last one is the diagnosis: the question is not "which OS is this" but "which
# INSTALL is this". The OS stays a separate question and is not folded in —
# `winget`/`brew`/`apt` is about which package manager a machine has, which is true of a
# checkout and an installed copy alike (`cli._exiftool_hint_key`).

# A copy the Windows installer put on the disk: a manifest next to the program, the
# interpreter and `uv` shipped with it, `sorta-setup` (the Start menu item) as the way to
# add tiers. No project directory and no `dev` extra here.
...
def install_kind(manifest: dict | None = None, *,
    ...
    doctor does), so the file is not read twice in one command; an empty dict means "this
    machine has no install manifest" and is how a test states a checkout.

    A manifest wins over everything else: it is written by the build and names the
    interpreter the copy runs on. Below it, the presence of `pyproject.toml` above the
    package is what makes `uv sync` a real command rather than a sentence about somebody
    else's machine.
    """
...
def advice_key(base: str, kind: str | None = None) -> str:
    ...
    so a family cannot be half-registered: the catalog either has all three variants or
    fails the parity test in tests/test_i18n.py.
    """
